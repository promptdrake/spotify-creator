...
async function createSpotifyAccount(line) {
  ...
  await page.goto(config.offers_hongkong['2b'], { waitUntil: 'domcontentloaded' });
  console.log('[Server] Visiting 2b Link: ' + chalk.green(config.offers_hongkong['2b']));
  ...
  while (retryCount < config.retry_count) {
    ...
    if (page.url().includes('spotify.com/sg-en/purchase/success')) {
      console.log(chalk.green('[Server] ') + email + ' | ' + password + ' Successfully Claimed Offer 2 Month');
      const currentDate = new Date();
      const options = { day: 'numeric', month: 'long',year: 'numeric'  };
      const locale = 'id-ID';
      const date = currentDate.toLocaleDateString(locale, options);
      const message = `(+) New Offer 2 Month\n\nEmail: \`\`${email}\`\`\nPassword: \`\`${password}\`\`\n\nCreated in ${date}`;
...
async function askUserCount() {
  console.log(chalk.green.bold("[Server] Starting Auto Pay 2b"));
